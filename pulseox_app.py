...
def plot():
    ...
    source = bokeh.models.ColumnDataSource(data=dict(time_ms=[], Voltage1=[], Voltage2=[]))
    peak_time_source = bokeh.models.ColumnDataSource(data=dict(v1_peak_time_ms=[0], v2_peak_time_ms=[0]))
    
    p.line(source=source, x="time_ms", y="Voltage1", visible=True, line_color="#f46d43", legend_label = "RED LED")
    p.line(source=source, x="time_ms", y="Voltage2", visible=True, legend_label = "IR LED")
    p.legend.orientation = "horizontal"
    p.add_layout(p.legend[0], 'above')
    
    # Put a phantom circle so axis labels show before data arrive
    phantom_source = bokeh.models.ColumnDataSource(data=dict(time_ms=[0], Voltage1=[7], Voltage2=[7]))
    ...
    heartrate_display = bokeh.models.Div(text="--", style={'font-size': '700%', 'color': '#3182bd', 'font-family': 'Helvetica'})
    
    oxygen_source = bokeh.models.ColumnDataSource(data=dict(oxygen=[]))
    oxygen_display = bokeh.models.Div(text="--", style={'font-size': '700%', 'color': '#3182bd', 'font-family': 'Helvetica'})
    return p, source, peak_time_source, phantom_source, heartrate_source, heartrate_display, oxygen_source, oxygen_display

def controls(mode):
    acquire = bokeh.models.Toggle(label="stream", button_type="success", width=100)
    save_notice = bokeh.models.Div(
        text="<p>No streaming data saved.</p>", width=300
    )
...
def layout(p, ctrls):
    ...
    bottom = bokeh.layouts.row(
        ctrls["file_input"],bokeh.layouts.column(
            ctrls["save_notice"],
            ctrls["save"])
    )
    return bokeh.layouts.column(
        top, bottom, background="whitesmoke",
    )
...
def reset_callback(mode, data, source, peak_time_source, phantom_source, heartrate_source, oxygen_source, controls):
    # Turn off the stream
    if mode == "stream":
...
def save_callback(mode, data, controls):
    ...
    df = pd.DataFrame(data={"time (ms)": data["time_ms"], "Voltage1 (V)": data["Voltage1"], "Voltage2 (V)": data["Voltage2"]})
...
def stream_update(data, source, peak_time_source, phantom_source, heartrate_source, oxygen_source, rollover, heartrate_display, oxygen_display):
    ...
    if len(source.data["time_ms"]) > window:
        if source.data["time_ms"][-1] > (peak_time_source.data["v2_peak_time_ms"][-1] + 3):
            v1_peak_times , v2_peak_times, v1_dc, v2_dc, time_window, oxygen, heartrates = take_snapshot(source, n=window)
            peak_time_source.stream({"v1_peak_time_ms": v1_peak_times, "v2_peak_time_ms": v1_peak_times}, rollover)

            # if there are peaks
            # DO SOMETHING
            heartrate_source.stream({"heartrate" : heartrates})
            heartrate_display.text = str(int(scipy.stats.trim_mean(heartrate_source.data["heartrate"], 0.1))) + "BPM"
            if oxygen[-1] != "--":
                oxygen_source.stream({"oxygen" : oxygen})
                oxygen_display.text = str(round(np.average(oxygen_source.data["oxygen"]),2)) + "%"
    # Adjust new phantom data point if new data arrived
    if len(new_data["time_ms"] > 0):
        phantom_source.data = dict(time_ms=[new_data["time_ms"][-1]], Voltage1=[new_data["Voltage1"][-1]], Voltage2=[new_data["Voltage2"][-1]])
...
def pulse_app(
    arduino, stream_data, daq_task, rollover=400, stream_plot_delay=1,
):
    def _app(doc):
        # Plots
        p_stream, stream_source, peak_time_source, stream_phantom_source, stream_heartrate, heartrate_display, stream_oxygen, oxygen_display= plot()
        ...
        app_layout = bokeh.layouts.column(bokeh.layouts.row(stream_layout, results_layout), background="whitesmoke")
        ...
        def _stream_update(step):
            stream_update(stream_data, stream_source, peak_time_source, stream_phantom_source, stream_heartrate, stream_oxygen, rollover, heartrate_display, oxygen_display)
